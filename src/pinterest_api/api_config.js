...
export class ApiConfig {
  constructor({ verbosity = 2 }) {
    ...
    this.landing_uri =
      process.env.REDIRECT_LANDING_URI || DEFAULT_LANDING_URI + this.app_id;

    this.oauth_token_dir =
      process.env.PINTEREST_OAUTH_TOKEN_DIR || DEFAULT_OAUTH_TOKEN_DIR;
  ...
  get_application_id() {
    ...
    if (this.app_id && this.app_secret) {
      if (this.verbosity >= 2) {
        console.log(
          `Using application ID and secret from ${env_app_id} and ${env_app_secret}.`
        );
      }
      return;
    }
    ...
    throw new Error(
      `${env_app_id} and ${env_app_secret} must be set in the environment.`
    );
  }

  credentials_warning() {
    console.log(
      "WARNING: This log has clear text credentials that need to be protected."
    );
  }
}
