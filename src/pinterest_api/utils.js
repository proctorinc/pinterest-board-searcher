...
export class Input {
  ...
  async number(prompt, minimum, maximum, defaultValue) {
    ...
    while (true) {
      ...
      if (minimum <= intval && intval <= maximum) {
        return intval;
      }
  ...
  async path_for_write(prompt, defaultFile) {
    ...
    while (true) {
      // keep going until a valid path is found
      // get the path
      ...
      try {
        // exclusive write to see if the file exists
        ...
        if (err.code === "EEXIST") {
          ...
          if (
            (await this.one_of("Overwrite this file?", ["yes", "no"], "no")) ===
            "no"
          ) {
            continue; // user requested not to overwrite
          } else {
            try {
              // verify that file is otherwise writable
