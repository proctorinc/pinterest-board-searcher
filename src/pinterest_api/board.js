...
export class Board extends ApiObject {
  ...
  async get_sections(query_parameters) {
    return this.get_iterator(
      `/v5/boards/${this.board_id}/sections`,
      query_parameters
    );
  }
  ...
  async get_section_pins(section_id, query_parameters) {
    return this.get_iterator(
      `/v5/boards/${this.board_id}/sections/${section_id}/pins`,
      query_parameters
    );
  }
  ...
  async create(board_data) {
    const OPTIONAL_ATTRIBUTES = ["description", "privacy"];
    const create_data = {
